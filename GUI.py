...
class GUI:
    def __init__(self):
        ...
        row += 1
        lbl_info = Label(self.window, justify=LEFT, text="Select a game in the list:").grid(sticky = W, column=0, row=row, columnspan=2)
        row += 1
        ...
        for idx, game in enumerate(self.fin.user.games):
            cbox = Checkbutton(self.window, text = "{} ({})".format(game.title, game.id), variable=self.select_value, onvalue=idx+1, offvalue=0, command=self.game_select)
